...
const ATTRIBUTIONS: Record<string, ScoreAttribution> = {
  apple: {
    summary: "+9 pts over 7d, driven mainly by Taiwan Strait exposure",
    drivers: [
  ...
  tsmc: {
    summary: "+12 pts from fab corridor stress and the geopolitical index",
    drivers: [
  ...
  foxconn: {
    summary: "+6 pts from SEA port congestion on assembly routes",
    drivers: [
...
export function getScoreAttribution(companyId: string, delta7d: string): ScoreAttribution {
  ...
  return {
    summary: `${delta7d} from composite signal drift across live streams`,
    drivers: [
